...
public class SequenceFile {
  ...
  /** Write key/value pairs to a sequence-format file. */
  public static class TrippleWriter implements java.io.Closeable {
    Configuration conf;
    FSDataOutputStream out;
    boolean ownOutputStream = true;
    DataOutputBuffer buffer = new DataOutputBuffer();
    ...
    protected Serializer t1Serializer;
    protected Serializer uncompressedt2Serializer;
    protected Serializer compressedt2Serializer;
    protected Serializer uncompressedt3Serializer;
    protected Serializer compressedt3Serializer;
    ...
    /** Create the named file. */
    public TrippleWriter(FileSystem fs, Configuration conf, Path name, 
                  Class t1Class, Class t2Class, Class t3Class)
      throws IOException {
      this(fs, conf, name, t1Class, t2Class, t3Class, null, new Metadata());
    }
    ...
    public TrippleWriter(FileSystem fs, Configuration conf, Path name, 
                  ...
                  Progressable progress, Metadata metadata)
      throws IOException {
      this(fs, conf, name, t1Class, t2Class, t3Class,
           fs.getConf().getInt("io.file.buffer.size", 4096),
           fs.getDefaultReplication(), fs.getDefaultBlockSize(),
           progress, metadata);
    }
    ...
    public TrippleWriter(FileSystem fs, Configuration conf, Path name,
      ...
      init(name, conf,
           fs.create(name, true, bufferSize, replication, blockSize, progress),
              t1Class, t2Class, t3Class, false, null, metadata);
      initializeFileHeader();
      writeFileHeader();
      finalizeFileHeader();
    }
    ...
    /** Write to an arbitrary stream using a specified buffer size. */
    TrippleWriter(Configuration conf, FSDataOutputStream out, 
                   Class t1Class, Class t2Class, Class t3Class, Metadata metadata)
      throws IOException {
      this.ownOutputStream = false;
      init(null, conf, out, t1Class, t2Class, t3Class, false, null, metadata);
    ...
    TrippleWriter ownStream() { this.ownOutputStream = true; return this; }
    
    /** Write and flush the file header. */
    void writeFileHeader() 
      throws IOException {
      Text.writeString(out, t1Class.getName());
      Text.writeString(out, t2Class.getName());
      Text.writeString(out, t3Class.getName());
    ...
    void init(Path name, Configuration conf, FSDataOutputStream out,
      ...
      this.uncompressedt2Serializer = serializationFactory.getSerializer(t2Class);
      this.uncompressedt2Serializer.open(buffer);
      this.uncompressedt3Serializer = serializationFactory.getSerializer(t3Class);
      this.uncompressedt3Serializer.open(buffer);
      if (this.codec != null) {
        ReflectionUtils.setConf(this.codec, this.conf);
        this.compressor = CodecPool.getCompressor(this.codec);
        this.deflateFilter = this.codec.createOutputStream(buffer, compressor);
        this.deflateOut = 
          new DataOutputStream(new BufferedOutputStream(deflateFilter));
        this.compressedt2Serializer = serializationFactory.getSerializer(t2Class);
        this.compressedt2Serializer.open(deflateOut);
        this.compressedt3Serializer = serializationFactory.getSerializer(t3Class);
        this.compressedt3Serializer.open(deflateOut);
      }
    }
    
    /** Returns the class of keys in this file. */
    public Class getT1Class() { return t1Class; }

    /** Returns the class of values in this file. */
    public Class getT2Class() { return t2Class; }
    
    /** Returns the class of values in this file. */
    public Class getT3Class() { return t3Class; }
    ...
    public synchronized void close() throws IOException {
      t1Serializer.close();
      uncompressedt2Serializer.close();
      uncompressedt3Serializer.close();
      if (compressedt2Serializer != null) {
        compressedt2Serializer.close();
      }
      if (compressedt3Serializer != null) {
	      compressedt3Serializer.close();
	    }
    ...
    /** Append a key/value pair. */
    public synchronized void append(Writable t1, Writable t2, Writable t3)
      throws IOException {
      append((Object) t1, (Object) t2, (Object) t3);
    }

    /** Append a key/value pair. */
    @SuppressWarnings("unchecked")
    public synchronized void append(Object t1, Object t2, Object t3)
      throws IOException {
      if (t1.getClass() != t1Class)
        throw new IOException("wrong t1 class: "+t1.getClass().getName()
                              +" is not "+t1Class);
      if (t2.getClass() != t2Class)
        throw new IOException("wrong t2 class: "+t2.getClass().getName()
                              +" is not "+t2Class);
      if (t3.getClass() != t3Class)
          throw new IOException("wrong t3 class: "+t3.getClass().getName()
                                +" is not "+t3Class);
      ...
      if (t1Length < 0)
        throw new IOException("negative length keys not allowed: " + t1);

      int t2Length = 0;
      // Append the 'value'
      if (compress) {
        deflateFilter.resetState();
        compressedt2Serializer.serialize(t2);
        t2Length = buffer.getLength();
        compressedt3Serializer.serialize(t3);
        deflateOut.flush();
        deflateFilter.finish();
      } else {
        uncompressedt2Serializer.serialize(t2);
        t2Length = buffer.getLength();
        uncompressedt3Serializer.serialize(t3);
      }

      // Write the record out
      checkAndWriteSync();                                // sync
      out.writeInt(buffer.getLength());                   // total record length
      out.writeInt(t1Length);                            // t1 portion length
      out.writeInt(t2Length);
      out.write(buffer.getData(), 0, buffer.getLength()); // data
    }

    /*
    public synchronized void appendRaw(byte[] t1Data, int t1Offset,
        int t1Length, ValueBytes val) throws IOException {
      if (keyLength < 0)
        throw new IOException("negative length keys not allowed: " + keyLength);
  ...
  /** Reads key/value pairs from a sequence-format file. */
  public static class TrippleReader implements java.io.Closeable {
    private Path file;
    private FSDataInputStream in;
    private DataOutputBuffer outBuf = new DataOutputBuffer();
    ...
    private String t1ClassName;
    private String t2ClassName;
    private String t3ClassName;
    private Class t1Class;
    private Class t2Class;
    private Class t3Class;
    ...
    private DataInputBuffer t1LenBuffer = null;
    private CompressionInputStream t1LenInFilter = null;
    private DataInputStream t1LenIn = null;
    private Decompressor t1LenDecompressor = null;
    private DataInputBuffer t1Buffer = null;
    private CompressionInputStream t1InFilter = null;
    private DataInputStream t1In = null;
    private Decompressor t1Decompressor = null;

    private DataInputBuffer t2LenBuffer = null;
    private CompressionInputStream t2LenInFilter = null;
    private DataInputStream t2LenIn = null;
    private Decompressor t2LenDecompressor = null;
    private DataInputBuffer t2Buffer = null;
    private CompressionInputStream t2InFilter = null;
    private DataInputStream t2In = null;
    private Decompressor t2Decompressor = null;
    
    private DataInputBuffer t3LenBuffer = null;
    private CompressionInputStream t3LenInFilter = null;
    private DataInputStream t3LenIn = null;
    private Decompressor t3LenDecompressor = null;
    private DataInputBuffer t3Buffer = null;
    private CompressionInputStream t3InFilter = null;
    private DataInputStream t3In = null;
    private Decompressor t3Decompressor = null;
    
    private Deserializer t1Deserializer;
    private Deserializer t2Deserializer;
    private Deserializer t3Deserializer;

    /** Open the named file. */
    public TrippleReader(FileSystem fs, Path file, Configuration conf)
      throws IOException {
      this(fs, file, conf.getInt("io.file.buffer.size", 4096), conf, false);
    }

    private TrippleReader(FileSystem fs, Path file, int bufferSize,
                   Configuration conf, boolean tempReader) throws IOException {
      this(fs, file, bufferSize, 0, fs.getLength(file), conf, tempReader);
    }
    
    private TrippleReader(FileSystem fs, Path file, int bufferSize, long start,
                   long length, Configuration conf, boolean tempReader) 
    throws IOException {
      this.file = file;
      this.in = openFile(fs, file, bufferSize, length);
    ...
    private void init(boolean tempReader) throws IOException {
      ...
      if (version < BLOCK_COMPRESS_VERSION) {
        ...
        className.readFields(in);
        t1ClassName = className.toString(); // t1 class name

        className.readFields(in);
        t2ClassName = className.toString(); // t2 class name
        
        className.readFields(in);
        t3ClassName = className.toString(); // t2 class name
      } else {
        t1ClassName = Text.readString(in);
        t2ClassName = Text.readString(in);
        t3ClassName = Text.readString(in);
      }
      ...
      // Initialize... *not* if this we are constructing a temporary Reader
      if (!tempReader) {
        t2Buffer = new DataInputBuffer();
        if (decompress) {
          t2Decompressor = CodecPool.getDecompressor(codec);
          t2InFilter = codec.createInputStream(t2Buffer, t2Decompressor);
          t2In = new DataInputStream(t2InFilter);
          t3Decompressor = CodecPool.getDecompressor(codec);
          t3InFilter = codec.createInputStream(t3Buffer, t3Decompressor);
          t3In = new DataInputStream(t3InFilter);
        } else {
          t2In = t2Buffer;
          t3In = t3Buffer;
        }
        ...
        if (blockCompressed) {
          ...
          t1LenBuffer = new DataInputBuffer();
          t1Buffer = new DataInputBuffer();
          t2LenBuffer = new DataInputBuffer();

          t1LenDecompressor = CodecPool.getDecompressor(codec);
          t1LenInFilter = codec.createInputStream(t1LenBuffer, 
                                                   t1LenDecompressor);
          t1LenIn = new DataInputStream(t1LenInFilter);

          t1Decompressor = CodecPool.getDecompressor(codec);
          t1InFilter = codec.createInputStream(t1Buffer, t1Decompressor);
          t1In = new DataInputStream(t1InFilter);

          t2LenDecompressor = CodecPool.getDecompressor(codec);
          t2LenInFilter = codec.createInputStream(t2LenBuffer, 
                                                   t2LenDecompressor);
          t2LenIn = new DataInputStream(t2LenInFilter);
        ...
        SerializationFactory serializationFactory =
          new SerializationFactory(conf);
        this.t1Deserializer =
          getDeserializer(serializationFactory, getT1Class());
        if (!blockCompressed) {
          this.keyDeserializer.open(valBuffer);
        } else {
          this.keyDeserializer.open(keyIn);
    ...
    public synchronized void close() throws IOException {
      // Return the decompressors to the pool
      CodecPool.returnDecompressor(t1LenDecompressor);
      CodecPool.returnDecompressor(t1Decompressor);
      CodecPool.returnDecompressor(t2LenDecompressor);
      CodecPool.returnDecompressor(t2Decompressor);
      CodecPool.returnDecompressor(t3LenDecompressor);
      CodecPool.returnDecompressor(t3Decompressor);
      t1LenDecompressor = t1Decompressor = null;
      t2LenDecompressor = t2Decompressor = null;
      t3LenDecompressor = t3Decompressor = null;
      
      if (t1Deserializer != null) {
    	t1Deserializer.close();
      }
      if (t2Deserializer != null) {
        t2Deserializer.close();
      }
      if (t3Deserializer != null) {
	    t3Deserializer.close();
	  }
    ...
    /** Returns the name of the key class. */
    public String getT1ClassName() {
      return t1ClassName;
    }

    /** Returns the class of keys in this file. */
    public synchronized Class<?> getT1Class() {
      if (null == t1Class) {
        try {
          t1Class = WritableName.getClass(getT1ClassName(), conf);
        } catch (IOException e) {
          throw new RuntimeException(e);
        }
      }
    ...
    /** Returns the name of the value class. */
    public String getT2ClassName() {
      return t2ClassName;
    }

    /** Returns the class of values in this file. */
    public synchronized Class<?> getT2Class() {
      if (null == t2Class) {
        try {
          t2Class = WritableName.getClass(getT2ClassName(), conf);
        } catch (IOException e) {
          throw new RuntimeException(e);
        }
      }
    ...
    /** Returns the name of the value class. */
    public String getT3ClassName() {
      return t3ClassName;
    }

    /** Returns the class of values in this file. */
    public synchronized Class<?> getT3Class() {
      if (null == t3Class) {
        try {
          t3Class = WritableName.getClass(getT3ClassName(), conf);
        } catch (IOException e) {
          throw new RuntimeException(e);
        }
      }
    ...
    //please modify the following, copied from Reader
