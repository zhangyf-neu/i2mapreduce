...
public class IterKmeans_BigCrossBack {

	public static class IntCenterKVInputFormat extends
			FileInputFormat<IntWritable, CenterWritable> implements
			JobConfigurable {
		...
		@Override
		public RecordReader<IntWritable, CenterWritable> getRecordReader(
	...
	public static class IntCenterKVLineRecordReader implements
			RecordReader<IntWritable, CenterWritable> {
		...
		public IntCenterKVLineRecordReader(Configuration job, InputStream in)
			...
			lineRecordReader = new LineRecordReader(in, 0, Integer.MAX_VALUE,
					job);
			dummyKey = lineRecordReader.createKey();
			innerValue = lineRecordReader.createValue();
			String sepStr = job.get("key.value.separator.in.input.line", "\t");
	...
	public static class DistributeDataMap extends MapReduceBase implements
		...
		public void map(Text key, Text value,
			...
			output.collect(new IntWritable(Integer.parseInt(key.toString())),
					value);
		}
	}
	...
	public static class DistributeDataReduce extends MapReduceBase implements
		...
		public void reduce(IntWritable key, Iterator<Text> values,
			 ...
			 * Text value = new Text(); while(values.hasNext()) { value =
			 * values.next(); String[] items =
			 * (((Text)values.next()).toString()).split(","); int dim_id =
			 * Integer.parseInt(items[0]); int dim_value =
			 * Integer.parseInt(items[1]);
			 * 
			 * if(dims.containsKey(dim_id)){ dims.put(dim_id, dims.get(dim_id) +
			 * dim_value); }else{ dims.put(dim_id, dim_value); } }
			 * 
			 * StringBuilder builder = new StringBuilder();
			 * for(Map.Entry<Integer, Integer> entry : dims.entrySet()){ int
			 * dim_id = entry.getKey(); int dim_value = entry.getValue();
			 * 
			...
			while (values.hasNext()) {
				...
				if (Util.getTaskId(conf) == 0 && (initCenters.size() < k)
						&& (key.get() % 2 == 0)) {
					...
					for (String item : items) {
						...
						double dim_value = Double.parseDouble(item
								.substring(index + 1));
						center.put(dim_id, dim_value);
					}
					initCenters.put(new IntWritable(centerIndex),
							new CenterWritable(center));
					centerIndex++;
				}
			}
		}
	...
	public static class KmeansMap extends MapReduceBase
			implements
		...
		private double distance(TreeMap<Integer, Double> first,
				TreeMap<Integer, Float> second) {
			double euclidean = 0;

			for (Map.Entry<Integer, Float> entry : second.entrySet()) {
				Double dim_value1 = first.get(entry.getKey());
				if (dim_value1 != null) {
					euclidean += (dim_value1 - entry.getValue())
							* (dim_value1 - entry.getValue());
				}
			}
		...
		public void map(IntWritable statickey, Text staticval,
			...
			if (centers == null) {
				...
				for (Map.Entry<Writable, Writable> entry : rawcenters
						.entrySet()) {
					int centerid = ((IntWritable) entry.getKey()).get();
					TreeMap<Integer, Double> centerdata = ((CenterWritable) entry
							.getValue()).get();
			...
			while (st.hasMoreTokens()) {
				...
				float dim_value = Float
						.parseFloat(element.substring(index + 1));
			...
			int simcluster = -1;
			for (Map.Entry<Integer, TreeMap<Integer, Double>> mean : centers
					.entrySet()) {
			...
			if (simcluster == -1) {
				System.out.println("simcluster is -1 " + statickey + "\t"
						+ simcluster + "\tvalue " + staticval);
			}
	...
	public static class KmeansReduce extends MapReduceBase implements
		...
		@Override
		public void reduce(IntWritable key, Iterator<Text> values,
				OutputCollector<IntWritable, CenterWritable> output,
				Reporter report) throws IOException {

			// input key: cluster's mean (whose mean has the nearest measure
			// distance)
			...
			while (values.hasNext()) {
				...
				while (st.hasMoreTokens()) {
					...
					int dim_id = Integer.parseInt(element.substring(0, index));
					float dim_value = Float.parseFloat(element
							.substring(index + 1));
					...
					if (oldv == null) {
						...
						all_items
								.put(dim_id, all_items.get(dim_id) + dim_value);
					}
				}
				num++;
			}
		...
		public float distance(IntWritable key, CenterWritable prevV,
			...
			if (prev.size() != curr.size()) {
				throw new IOException("size doesn't match! " + prev.size()
						+ ":" + curr.size());
			}
		...
		public void iteration_complete(int iteration) {
			...
			System.out.println("iteration " + iteration + " takes "
					+ (curr_time - last_iter_end) + " total "
					+ (curr_time - iter_start));
			last_iter_end = curr_time;
		}

	}

	public static class KmeansProjector
			implements
			Projector<IntWritable, GlobalUniqKeyWritable, GlobalUniqValueWritable> {
		...
		@Override
		public void configure(JobConf job) {
			// the inputformat can only be retrieved in the iterative job, and
			// the initial value
			if (job.isIterative()) {
				try {
					...
					InputSplit inputSplit = new FileSplit(initcenterpath, 0,
							filelen, job);
					reader = job.getDynamicInputFormat().getRecordReader(
							inputSplit, job, null);
		...
		@Override
		public GlobalUniqValueWritable initDynamicV(
				GlobalUniqKeyWritable dynamickey) {
			return value;
		}
	...
	public static int main(String[] args) throws Exception {
		...
		JobConf job1 = new JobConf(IterKmeans_BigCrossBack.class);
		String jobname1 = "Kmeans Init";
		job1.setJobName(jobname1);

		job1.setDataDistribution(true);
		job1.setIterativeAlgorithmID(iteration_id); // must be unique for an
													// iterative algorithm
		...
		Util.writeLog("iter.kmeans.log", "init job use "
				+ (initend - initstart) / 1000 + " s");
		...
		JobConf job = new JobConf(IterKmeans_BigCrossBack.class);
		...
		job.setIterativeAlgorithmID(iteration_id); // must be unique for an
													// iterative algorithm
		...
		job.setDynamicInputFormat(GlobalDataInputFormat.class); // MUST have
																// this for the
																// following
																// jobs, even
																// though the
																// first job not
																// need it
		// job.setResultInputFormat(IntCenterKVInputFormat.class); //for kmeans,
		// we don't want to check the threshold, we only set max iterations
		...
		Util.writeLog("iter.kmeans.log", "iteration computation takes "
				+ itertime + " s");
