  ...
  private static SplitMetaInfo[] writeOldSplits(
    ...
    if (splits.length != 0) {
      ...
      for(org.apache.hadoop.mapred.InputSplit split: splits) {
        ...
        //append the split index information for iterative jobs
        if(((JobConf)conf).isIterative()){
        	String filename = ((FileSplit)split).getPath().getName();
        	//LOG.info("the file name is " + filename);
        	int index = -1;
			try {
				index = NUMBER_FORMAT.parse(filename.substring(filename.indexOf("part-") + 5)).intValue();
			} catch (ParseException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
        	info[i-1].setSplitIndex(index);
        }
