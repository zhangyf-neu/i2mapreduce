...
class MapOutputFile {
  ...
  /**Yanfeng,
   * Return the path to local state data file created earlier
   * 
   * @return path
   * @throws IOException
   */
  public Path getIterativeStateFile()
      throws IOException {
    return lDirAlloc.getLocalPathToRead(TaskTracker.OUTPUT + Path.SEPARATOR
        + "iterative.state.file", conf);
  }
  
  /**Yanfeng,
   * Return the path to local static data file created earlier
   * 
   * @return path
   * @throws IOException
   */
  public Path getIterativeStaticFile()
      throws IOException {
    return lDirAlloc.getLocalPathToRead(TaskTracker.OUTPUT + Path.SEPARATOR
        + "iterative.static.file", conf);
  }
  ...
  /** Yanfeng
   * Create a local state data file name.
   * 
   * @param size the size of the file
   * @return path
   * @throws IOException
   */
  public Path getIterativeStateFileForWrite(long size)
      throws IOException {
    return lDirAlloc.getLocalPathForWrite(TaskTracker.OUTPUT + Path.SEPARATOR
        + "iterative.state.file", size, conf);
  }
  
  /** Yanfeng
   * Create a local static data file name.
   * 
   * @param size the size of the file
   * @return path
   * @throws IOException
   */
  public Path getIterativeStaticFileForWrite(long size)
      throws IOException {
    return lDirAlloc.getLocalPathForWrite(TaskTracker.OUTPUT + Path.SEPARATOR
        + "iterative.static.file", size, conf);
  }
