...
class TaskInProgress {
  ...
  synchronized boolean updateStatus(TaskStatus status) {
    ...
    iteration = status.getIteration();		//added by Yanfeng, 2014.12.28
    ...
    LOG.info("tracking the counters");
    for(Statistics stat: FileSystem.getAllStatistics()) {
         ...
         Counters.Counter c = status.getCounters().findCounter("FileSystemCounters", counterNames[0]);
          LOG.info("sending counters update " + taskid.getJobID() + "\t" + c.getName() + "\t" + c.getValue());
      }
