...
public class JobInProgress {
  ...
  // the map task is killed, original is 3
  //changed by Yanfeng Zhang
  private static final int MAX_FETCH_FAILURES_NOTIFICATIONS = 1024;
  ...
  public synchronized boolean scheduleReduces() {
	  ...
	  if(this.conf.isIncrementalIterative() || this.conf.isIterative()
			  || conf.isIterCPC() || conf.isIncrMRBGOnly()){
		  return true;
	  }
  ...
  public synchronized Task obtainFailedReduceTask(TaskTrackerStatus tts,
          int clusterSize,
          int numUniqueHosts
         ) throws IOException {
	    if (status.getRunState() != JobStatus.RUNNING) {
	      ...
	      int  target = findFailedReduceTask(tts, clusterSize, numUniqueHosts, 
  ...
  private synchronized void failMap(TaskInProgress tip) {
    ...
    LOG.debug("add failed task " + tip.getTIPId());
    failedMaps.add(tip);
  }
  ...
  private synchronized TaskInProgress findTaskFromList(
    ...
    while (iter.hasNext()) {
      ...
      //LOG.info("i am in find task from list " + tip + "\t" + tip.getIdWithinJob() + "\t" + tip.isRunnable() + "\t" + tip.isRunning());
      // Select a tip if
      ...
      //LOG.info("tip is runnable " + tip.isRunnable() + "\t tip is running " + tip.isRunning());
      //LOG.info("tip has failed on " + ttStatus.getHost() + "\t " + tip.hasFailedOnMachine(ttStatus.getHost())
    		  //+ " tip.getNumberOfFailedMachines() " + tip.getNumberOfFailedMachines() + " " + numUniqueHosts);
  ...
  private synchronized int findNewMapTask(final TaskTrackerStatus tts, 
    ...
    //for iterative failed task recovery
    for(TaskInProgress tipt : failedMaps){
    	LOG.info("failed task " + tipt.getTIPId());
    }
    if(this.getJobConf().isIterative() || this.getJobConf().isIncrementalIterative()){
    	Iterator<TaskInProgress> iter = failedMaps.iterator();
        while (iter.hasNext()) {
          TaskInProgress tip2 = iter.next();
          failedMaps.remove(tip2); 
          scheduleMap(tip2);
          LOG.info("Choosing a failed task " + tip2.getTIPId());
          return tip2.getIdWithinJob();
        }
    }
    ...
    if (node != null) {
      ...
      if (level == maxCacheLevel) {
    	  LOG.info("non running tip on " + key.getName());
        return -1;
      }
    }
  ...
  private synchronized int findFailedReduceTask(TaskTrackerStatus tts, 
          int clusterSize,
          int numUniqueHosts,
          double avgProgress) {
		if (numReduceTasks == 0) {
	    ...
	    Iterator<TaskInProgress> iter = nonRunningReduces.iterator();
	    while (iter.hasNext()) {
	      tip = iter.next();
